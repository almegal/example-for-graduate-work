...
public class User {
    ...
    @ApiModelProperty(value = "Email пользователя", example = "user@example.com")
    ...
    @ApiModelProperty(value = "Фамилия пользователя", example = OpenApiConstant.LAST_NAME)
    private String lastName;

    @NotBlank(message = "Телефон должен быть валидным номером в формате +7 (XXX) XXX-XX-XX")
    @Pattern(regexp = "\\+7\\s?\\(?\\d{3}\\)?\\s?\\d{3}-?\\d{2}-?\\d{2}", message = "Телефон должен быть валидным номером")
    @ApiModelProperty(value = "Телефон пользователя", example = OpenApiConstant.PHONE)
    private String phone;

    @NotNull(message = "Роль не может быть null")
    @ApiModelProperty(value = "Роль пользователя", example = "USER")
    private Role role;
